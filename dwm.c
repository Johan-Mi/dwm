...
enum { SchemeNorm, SchemeSel, SchemeStatus, SchemeTagsSel, SchemeTagsNorm, SchemeInfoSel, SchemeInfoNorm, SchemeLayout }; /* color schemes */
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - tw - stw, 0, sw, bh, lrpad / 2, stext, 0);
...
void
runAutostart(void) {
	system("pgrep dwmblocks >/dev/null || exec dwmblocks &");
}
